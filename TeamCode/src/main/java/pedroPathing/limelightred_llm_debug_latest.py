...
def draw_info(image, color, angle, center, index, area, frame_width=320, frame_height=240):
    ...
    cv2.putText(image, f"#{index}: {color}", (center[0] - 50, center[1] - 80), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
    cv2.putText(image, f"Angle: {angle:.1f}°", (center[0] - 50, center[1] - 65), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
    cv2.putText(image, f"Area: {area:.0f}", (center[0] - 50, center[1] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
    cv2.putText(image, f"Dist: {distance:.1f}in", (center[0] - 50, center[1] - 35), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
    cv2.putText(image, f"X:{robot_pos['x_robot']:.1f} Y:{robot_pos['y_robot']:.1f}", (center[0] - 50, center[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1)
    cv2.putText(image, f"Turn: {robot_pos['angle_to_sample']:.1f}°", (center[0] - 50, center[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1)
...
def runPipeline(frame, llrobot):
    try:
        ...
        for i, contour in enumerate(red_contours):
            ...
            for sep_contour in separate_touching_contours(contour):
                ...
                if area > largest_area:
                    ...
                    llpython = [1, center[0], center[1], angle, len(red_contours), 0, 0, 0]
        ...
        cv2.putText(frame, f"Red Pipeline | Samples: {len(valid_contours)}", (5, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)
