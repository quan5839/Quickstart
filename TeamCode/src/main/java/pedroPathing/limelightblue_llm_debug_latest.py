...
# Import camera constants for distance calculations and robot positioning
from camera_constants import (
    calculate_distance_to_sample,
    calculate_robot_relative_position,
    get_camera_info_string,
    CAMERA_WIDTH as CAM_WIDTH,
    CAMERA_HEIGHT_PIXELS as CAM_HEIGHT
)
...
def draw_info(image, color, angle, center, index, area, frame_width=320, frame_height=240):
    # Calculate distance and robot-relative position using camera constants
    distance = calculate_distance_to_sample(center[0], center[1], frame_width, frame_height)
    robot_pos = calculate_robot_relative_position(center[0], center[1], frame_width, frame_height)

    # Draw sample information with distance data
    cv2.putText(image, f"#{index}: {color}", (center[0] - 50, center[1] - 80), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
    cv2.putText(image, f"Angle: {angle:.1f}°", (center[0] - 50, center[1] - 65), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
    cv2.putText(image, f"Area: {area:.0f}", (center[0] - 50, center[1] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
    cv2.putText(image, f"Dist: {distance:.1f}in", (center[0] - 50, center[1] - 35), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
    cv2.putText(image, f"X:{robot_pos['x_robot']:.1f} Y:{robot_pos['y_robot']:.1f}", (center[0] - 50, center[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 0, 0), 1)
    cv2.putText(image, f"Turn: {robot_pos['angle_to_sample']:.1f}°", (center[0] - 50, center[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 0, 0), 1)
...
def runPipeline(frame, llrobot):
    try:
        ...
        #cv2.imshow('3. Blue Mask', blue_mask)
        #return debug_return(blue_mask)
        # Process blue color
        ...
        for contour_info in valid_contours:
            ...
            draw_info(frame, "Blue", contour_info['angle'], contour_info['center'],
                     contour_info['index'] + 1, contour_info['area'], frame_width, frame_height)

        # Add camera configuration info at top of frame
        camera_info = get_camera_info_string()
        cv2.putText(frame, camera_info, (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)
        cv2.putText(frame, f"Blue Pipeline | Samples: {len(valid_contours)}", (5, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)
